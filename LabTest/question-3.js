...
currentLog = fs.appendFile('./logs/mylog10.txt', 'log number 5', (error) => {
  if (error) throw error;
    console.log('log was saved!');
});

const readLog = fs.readdirSync(logDir);
console.log('The logs are these ones', readLog)


// how i think the removing works 
removingLog = fs.rmSync('./logs/mylog5.txt')

// i wasnt able to get the foreach to work
